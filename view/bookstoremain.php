...
if(isset($_POST['searchName'])){
	$dbsearchlist = $DBbook->DbsearchBook("book", "title", $_POST['searchName'], "*");
	$searchrows = mysqli_fetch_assoc($dbsearchlist);
	
	$searchName = $searchrows['title'];
	$localdir = preg_replace("/\\\\/", "/", $searchrows['path']);
	$directory = $_SERVER['DOCUMENT_ROOT'] . $localdir;
	$dirname = $searchrows['writer'];
}
//파일 선택 시 불러오기
...
//전체 도서 데이터 조회
$dbbooklist = $DBbook->DbBookList("book", "*");
    ...
    <body>
        ...
        function updatepage(filename) {
            ...
            if(extension == "txt"){
				...
				vimg = $('<img>', { 
                 	'src' : './../..'+'<?php echo $localdir?>' +'/'+filename,
                  	 'width' : '100%',
                     'height' : '100%'
              	 });
            	$(vimg).appendTo('#init'); 
        ...
        $(document).ready(function () {
            ...
            //단어 검색
             $(document).on('click', '#searchtextbtn',function(){
	            ...
	            $('.pagetext').html($('.pagetext').html().replace(RegExp(searchtext,'gi'),"<span class='bg-primary-subtle'>"+searchtext+"</span>"));	
